...
sed -i '$a <flags CXXFLAGS="-Wno-error=format-overflow"/>' "$BUILDDIR/src/LCG/CoralBase/BuildFile.xml"
sed -i 's/<use   name="python3"\/>/<use   name="Python"\/>/' "$BUILDDIR/src/LCG/PyCoral/BuildFile.xml"
sed -i 's/PyUnicode_GET_SIZE/PyUnicode_GET_LENGTH/g' "$BUILDDIR/src/LCG/PyCoral/src/Attribute.cpp"
